...
class Feed extends React.Component {
            ...
            <div key={board.id} id="board-show-link">
              <div className="board-display-card mod-feed-card">
                ...
                <div className="title-pin-section mod-feed-title-section">
                  <h2 className="board-title mod-feed-board-title">
  ...
  componentDidMount() {
    ...
    this.props.numFollowing(this.props.currentUser.id).then((data) => {
      this.setState({ showFollowOpt: this.props.numFollowers });
    });

    this.props.fetchFeedPins(this.props.currentUser.id).then((data) => {
      data;
  ...
  selectFollow(e) {
    ...
    if (userBox.classList.length >= 2) {
      ...
      this.props.unfollowUser(delteIds).then((data) => {
      ...
      this.props.createFollow(followForm).then((data) => {
                  ...
                  {users.map((user) => (
                    <li key={user.id}>
                      <div
                        className={
                          user.followThisUser
                            ? "outside-border-layer toggle-border"
                      ...
                      >
                        <div
                          className={
                            user.followThisUser
                              ? "list-div-feed selected-follow-feed"
                              : "list-div-feed"
                          }
                          id={user.id}
                          onClick={this.selectFollow}
                        >
                          <div className="profile-icon-feed">
                            <div
                              id="123 E"
                              className="div-for-search-user-img mod-feed-img"
                            >
                              {!(user.photoUrl === "false") ? (
                                <img
                          ...
                          <div className="div-pin-boards">
                            {!!user.boards ? (
                              this.displayTheseBoards(
                                Object.values(user.boards)
                              )
                            ) : (
                              <div className="no-board-yet-feed">
                                This user hasn't created a board yet :/
                              </div>
                            )}
                          </div>
                        </div>
