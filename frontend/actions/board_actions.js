...
export const fetchBoards = (userId) => {
  return (dispatch) => {
    return APIUtil.fetchBoards(userId).then((boards) =>
      dispatch(receiveBoards(boards))
    ); // posibly add in error handeling
  };
};

export const fetchBoard = (boardId) => {
  return (dispatch) => {
    return APIUtil.fetchBoard(boardId).then((board) =>
      dispatch(receiveBoardShow(board))
    );
  };
};
...
export const createNewBoard = (formBoard) => {
  return (dispatch) => {
    return APIUtil.createNewBoard(formBoard).then((board) =>
...
export const deleteBoard = (boardId) => {
  return (dispatch) => {
    return APIUtil.deleteBoard(boardId).then((deletedInfo) =>
      dispatch(deletedBoard(deletedInfo))
    );
  };
};
